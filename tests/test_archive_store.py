...
from ercot_mis.archive import store
...
def test_store_is_content_addressed_read_only_and_deduplicated(tmp_path):
    ...
    first = store.put(tmp_path, "NP7-800-M", [data[:5], data[5:]], ".zip")
    ...
    again = store.put(tmp_path, "NP7-800-M", [data], ".zip")
...
def test_interrupted_store_leaves_nothing(tmp_path):
    ...
    with pytest.raises(ConnectionError):
        store.put(tmp_path, "NP7-800-M", chunks(), ".zip")
    assert list((tmp_path / "archive" / ".partial").iterdir()) == []
...
def test_index_members_hashes_files_without_extracting(tmp_path):
    ...
    members = {m.member_path: m for m in store.index_members(path)}
    ...
    assert store.index_members(plain) == []
